<template>
  <div class="check-goods-detail">
    <mt-loadmore 
      ...
      ref="loadmore"
      >
      <div class="list border-1px" v-for="(list,index) in goods.data" :key="index">
        <div class="left">
          <div>本期订货：{{list.number}}吨</div>
          <div>本期已发：{{list.sum_delivery_number}}吨</div>
        </div>
        <div class="right">
          <div>{{list.type}}</div>
          <div>{{list.settlement_price}}元*{{list.number}}吨</div>
        </div>
      </div>
...
<script>
import {goodsDetail} from '@/data/getData';
export default {
  data () {
    return {
      goods: {
  ...
  methods: {
    ...
    async loadBottom () {
      ...
      for (let i = 0; i < res.data.list.length; i++) {
        this.goods.data.push(res.data.list[i]);
      }
      this.$nextTick(() => {
        this.$refs.loadmore.onBottomLoaded();
      });
